...
void CgiHandler::_initEnv(Request &request, GetConf &getConf) {
    ...
    std::map<std::string, std::string> headers =
        request.getHeaders(); // request에서 '요청헤더' 가져옴

    this->_env["SCRIPT_NAME"] =
        getConf.getContentLocation(); // HTTP 요청의 첫 번째 라인에 있는 조회
                                      // 문자열까지의 URL.
    this->_env["REQUEST_METHOD"] = request.getMethod();
    this->_env["PATH_INFO"] =
        request.getTargetPath(); // might need some change, using config
                                 // path/contentLocation
    this->_env["PATH_TRANSLATED"] =
        request.getTargetPath(); // might need some change, using config
                                 // path/contentLocation //진짜경로
    this->_env["REQUEST_URI"] =
        request.getTargetPath() +
        request.getQuery(); //현재 페이지 주소에서 도메인을 제외한 값.
    this->_env["SERVER_PROTOCOL"] = "HTTP/1.1";
    this->_env.insert(getConf.getCgiParam().begin(),
                      getConf.getCgiParam().end());
}
...
CgiHandler::~CgiHandler(void) { return; }
