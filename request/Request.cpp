...
Request::Request()
    : _method(""), _target_path(""), _query(""), _version(""), _body(""),
      _status_code(200) {
    initHeaders();
    initValidMethod();
}
...
Request::Request(const Request &_Request) { *this = _Request; }
...
void Request::parseProcess(std::string &request_value) {
    ...
    while ((line = readLine(request_value, i)) != "\r" && line != "" &&
           _status_code != 400) {
        // key 읽어줌
        key = readKey(line);
        // value 읽어줌
    ...
    if (this->_headers["Www-Authenticate"] != "")
        ...
        this->_env_for_cgi["Www-Authenticate"] =
...
void Request::parseMethod(const std::string &line, size_t &i) {
    // line에서 띄어쓰기 위치 찾고 띄어쓰기 없으면 400 에러(버전이 없다는
    // 뜻이니까)
...
void Request::parsePath(const std::string &line, size_t &i) {
    size_t start;
    // i가 띄어쓰기가 아니면 PATH나 HTTP 버전 없는거니까 에러, 띄어쓰기 여러개일
    // 경우 제거
...
void Request::parseVersion(const std::string &line, size_t &i) {
    ...
    if (_version != "1.0" && _version != "1.1") {
        _status_code = 400;
        std::cerr << "Request ERR:: UNSUPPORTED HTTP VERSION (" << _version
                  << ")" << std::endl;
        return;
    }
}
...
void Request::checkMethod() {
    // 기존 map에 정리해놓은 _valid_methods안에 우리가 파싱한 method 있는 지
    // 확인 후 리턴
...
const std::string &Request::getTargetPath() const { return this->_target_path; }
...
const int Request::getStatus() const { return this->_status_code; }
