...
void Response::responseSet(Request &request,
                           GetConf &getconf) //(request, getconf ) //받기
{
...
void Response::run(Request &request,
                   GetConf &getconf) //(request, getconf ) //받기
{
...
void Response::getMethod(Request &request, GetConf &getconf) {
    ...
    if (_status_code == 500) //예측하지 못한 서버에러
        _response = this->readHtml(_error_map[_status_code]);
    _response =
        header.getHeader(_response.size(), _target_path, _location,
                         _status_code, _type, getconf.getContentLocation()) +
        "\r\n" + _response;
}
...
int Response::readContent(void) {
    ...
    if (pathIsFile(
            _target_path)) // path에 파일이 있는지 확인하기 1이면 파일, 나머지는
                           // 0, [path = root + target_target_path]
    {
        file.open(_target_path.c_str(), std::ifstream::in); //파일열기
        if (file.is_open() == false) {
            _response = this->readHtml(
                _error_map[403]); // 403읽고 반납, 작동중인 서버에 클라이언트의
                                  // 요청이 도달했으나, 서버가 클라이언트의
                                  // 접근을 거부할 때 반환하는 HTTP 응답
                                  // 코드이자 오류 코드이다. 이 에러는 서버 자체
                                  // 또는 서버에 있는 파일에 접근할 권한이 없을
                                  // 경우에 발생한다.
            return (403);
        }
    ...
    } else if (_auto_index) // 파일이 없으면 + 오토인덱스가 켜져있으면
    {
        int tmpHost = 0;
        int tmpPort = 80;
        buffer << AutoIndexGenerator::getPage(
            _target_path.c_str(), to_string(tmpHost),
            tmpPort); // HOST, PORT  정리되면 다시 넣어야함
...
int Response::writeContent(std::string content) {
    ...
    if (pathIsFile(_target_path)) {
        ...
        file << content;
        file.close();
        return (204); //흔히 204를 반환하는 경우는 PUT 요청에 대한 응답으로,
                      //사용자에게 보여지는 페이지를 바꾸지 않고 리소스를
                      //업데이트할 때 쓰입니다. 있는 파일에다가 쓸ㅇㅜ
    } else {
        file.open(_target_path.c_str(),
...
std::string Response::readHtml(const std::string &path) {
    ...
    if (pathIsFile(path)) {
        ...
        if (file.is_open() == false)
            return ("<!DOCTYPE html>\n<html><title>404</title><body>There was "
                    "an error finding your error page</body></html>\n");
        ...
        return ("<!DOCTYPE html>\n<html><title>404</title><body>There was an "
                "error finding your error page</body></html>\n");
}
...
void Response::checkClientError(Request &request, GetConf &getconf) {
    if (getconf.getAllowedMethods().find(request.getMethod()) ==
        getconf.getAllowedMethods().end())
        _status_code = 405;
...
void Response::setCgiResult(Request &request, GetConf &getconf) {
    ...
    _response =
        cgi.executeCgi(getconf.getCgiPass()); // cig결과값 _response에 넣기
...
void Response::initResponse(void) { _response = ""; }
