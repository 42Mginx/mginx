...
std::string ServerBlock::getRedirect() const { return this->_redirect; }
...
void ServerBlock::passMembers(ServerBlock &server) const {
    ...
    if (this != &server) {
        ...
        if (server._listen.empty())
            ...
            server._listen.insert(server._listen.begin(), this->_listen.begin(),
                                  this->_listen.end());
        // 서버 _root 비었니
        if (server._root == "")
            // this 루트 넣어줌
        ...
        server._server_name.insert(server._server_name.end(),
                                   this->_server_name.begin(),
                                   this->_server_name.end());
        // _error_page 넣어줌
        for (std::map<int, std::string>::const_iterator i =
                 this->_error_page.begin();
             i != this->_error_page.end(); i++) {
        ...
        for (std::map<std::string, std::string>::const_iterator i =
                 this->_cgi_param.begin();
             i != this->_cgi_param.end(); i++) {
    ...
    // _location 넣어줌
    for (std::map<std::string, ServerBlock>::iterator i =
             server._location.begin();
         i != server._location.end(); i++)
...
ServerBlock ServerBlock::getLocationForRequest(std::string const path,
    ...
    if (!this->_location.empty()) {
        do {
            ...
            if (iter != this->_location.end() && iter->first[0] != '*') {
                ...
                return iter->second.getLocationForRequest(
                    path, retLocationPath); // ServerBlock(serverblock)
            }
            tryLen--;
        } while (tryLen);
        for (std::map<std::string, ServerBlock>::iterator i =
