...
std::vector<t_listen> Config::parseAllListens() const {
    ...
    std::cout << "//testing parse all listens" << std::endl;
    // 서버블록을 순환

    std::cout << "//testing server blocks size: " << _serverBlocks.size()
              << std::endl;
    for (std::vector<ServerBlock>::const_iterator serverBlock =
             _serverBlocks.begin();
